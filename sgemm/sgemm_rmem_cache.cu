...
void sgemm_rmem_cache_kernel(float* A, float* B, float* C, float alpha, float beta, int M, int N, int K, int lda, int ldb, int ldc) {
    ...
    __shared__ float smem_tile_a[tile_row * tile_col];
    __shared__ float smem_tile_b[tile_col * tile_row];
    float frag_acc[frag_size * frag_size] = {};
    for (int tile_move_cnt = 0; tile_move_cnt * tile_col < K; tile_move_cnt += 1) {
